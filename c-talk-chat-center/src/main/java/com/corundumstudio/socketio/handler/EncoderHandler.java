...
public class EncoderHandler extends ChannelOutboundHandlerAdapter {
    ...
    private static final Logger log = LoggerFactory.getLogger(com.corundumstudio.socketio.handler.EncoderHandler.class);
    ...
    private void readVersion() throws IOException {
        ...
        while (resources.hasMoreElements()) {
            try (InputStream inputStream = resources.nextElement()
                                                    .openStream()) {
                ...
                if (name != null && name.equals("netty-com.corundumstudio.socketio")) {
    ...
    private void write(XHROptionsMessage msg, ChannelHandlerContext ctx, ChannelPromise promise) {
        ...
        res.headers()
           .add(HttpHeaderNames.SET_COOKIE, "io=" + msg.getSessionId() + "; Secure; SameSite=None")
           .add(HttpHeaderNames.CONNECTION, HttpHeaderValues.KEEP_ALIVE)
           .add(HttpHeaderNames.ACCESS_CONTROL_ALLOW_HEADERS, HttpHeaderNames.CONTENT_TYPE);

        String origin = ctx.channel()
                           .attr(ORIGIN)
                           .get();
    ...
    private void sendMessage(HttpMessage msg, Channel channel, ByteBuf out, String type, ChannelPromise promise, HttpResponseStatus status) {
        ...
        res.headers()
           .add(HttpHeaderNames.CONTENT_TYPE, type)
           .add(HttpHeaderNames.CONNECTION, HttpHeaderValues.KEEP_ALIVE);
        ...
        String origin = channel.attr(ORIGIN)
                               .get();
        addOriginHeaders(origin, res);
        ...
        String userAgent = channel.attr(com.corundumstudio.socketio.handler.EncoderHandler.USER_AGENT)
                                  .get();
    ...
    private void sendError(HttpErrorMessage errorMsg, ChannelHandlerContext ctx, ChannelPromise promise) throws IOException {
        ...
        encoder.getJsonSupport()
               .writeValue(out, errorMsg.getData());
    ...
    private void addOriginHeaders(String origin, HttpResponse res) {
        if (version != null) {
            res.headers()
               .add(HttpHeaderNames.SERVER, version);
        }

        if (configuration.getOrigin() != null) {
            res.headers()
               .add(HttpHeaderNames.ACCESS_CONTROL_ALLOW_ORIGIN, configuration.getOrigin());
            res.headers()
               .add(HttpHeaderNames.ACCESS_CONTROL_ALLOW_CREDENTIALS, Boolean.TRUE);
        } else {
            if (origin != null) {
                res.headers()
                   .add(HttpHeaderNames.ACCESS_CONTROL_ALLOW_ORIGIN, origin);
                res.headers()
                   .add(HttpHeaderNames.ACCESS_CONTROL_ALLOW_CREDENTIALS, Boolean.TRUE);
            } else {
                res.headers()
        ...
        if (configuration.getAllowHeaders() != null) {
            res.headers()
               .add(HttpHeaderNames.ACCESS_CONTROL_ALLOW_HEADERS, configuration.getAllowHeaders());
        }
    }
    ...
    private void handleWebsocket(final OutPacketMessage msg, ChannelHandlerContext ctx, ChannelPromise promise) throws IOException {
        ...
        while (true) {
            Queue<Packet> queue = msg.getClientHead()
                                     .getPacketsQueue(msg.getTransport());
            ...
            for (ByteBuf buf : packet.getAttachments()) {
                ...
                writeFutureList.add(ctx.channel()
                                       .writeAndFlush(new BinaryWebSocketFrame(outBuf)));
            }
        }
    }
    ...
    private void handleHTTP(OutPacketMessage msg, ChannelHandlerContext ctx, ChannelPromise promise) throws IOException {
        ...
        ByteBuf out = encoder.allocateBuffer(ctx.alloc());
        Boolean b64 = ctx.channel()
                         .attr(com.corundumstudio.socketio.handler.EncoderHandler.B64)
                         .get();
        if (b64 != null && b64) {
            Integer jsonpIndex = ctx.channel()
                                    .attr(com.corundumstudio.socketio.handler.EncoderHandler.JSONP_INDEX)
                                    .get();
    ...
    private static class ChannelFutureList implements GenericFutureListener<Future<Void>> {
        ...
        private final List<ChannelFuture> futureList = new ArrayList<ChannelFuture>();
